...
impl Widget for HudButton {
    fn ui(self, ui: &mut Ui) -> Response {
        let size = if cfg!(target_os = "android") { 48.0 } else { 32.0 };
        ...
        if alpha > 0 {
            ui.painter().rect_filled(rect, 6.0, Color32::from_white_alpha(alpha));
        }
        ...
        let text_pos = Align2::CENTER_CENTER.align_size_within_rect(galley.size(), rect).min;
