...
static WEB_SHELL_MODE: OnceLock<bool> = OnceLock::new();

pub fn web_shell_mode() -> bool {
    *WEB_SHELL_MODE.get_or_init(|| {
        #[cfg(target_arch = "wasm32")]
        {
            take_window_bool("SOW_WEB_SHELL_MODE")
        }
        #[cfg(not(target_arch = "wasm32"))]
        {
            false
...
pub fn take_boot_action() -> Option<String> {
    #[cfg(target_arch = "wasm32")]
    {
        let val = get_window_value("SOW_BOOT_ACTION");
        if let Some(s) = val.as_ref().and_then(|v| v.as_string()) {
            if !s.is_empty() {
                call_window_hook("SOW_portalClearBootAction");
                return Some(s);
            }
        }
        None
...
pub fn drain_ui_commands() -> Vec<String> {
    #[cfg(target_arch = "wasm32")]
    {
        let mut commands = Vec::new();
        if let Some(arr_val) = get_window_value("SOW_UI_COMMANDS") {
            if js_sys::Array::is_array(&arr_val) {
                let arr = arr_val.unchecked_into::<js_sys::Array>();
                let len = arr.length();
                for i in 0..len {
                    let val = arr.get(i);
                    if let Ok(js_str) = js_sys::JSON::stringify(&val) {
...
pub fn read_settings_json() -> Option<String> {
    #[cfg(target_arch = "wasm32")]
    {
        get_window_value("SOW_SETTINGS_JSON").and_then(|v| v.as_string())
    }
    #[cfg(not(target_arch = "wasm32"))]
    {
        None
...
pub fn emit_match_end(json: &str) {
    call_window_hook_str("SOW_onMatchEnd", json);
}

pub fn emit_join_failed(reason: &str) {
    call_window_hook_str("SOW_onJoinFailed", reason);
}

pub fn emit_match_starting() {
    call_window_hook("SOW_onMatchStarting");
}

pub fn emit_lobby_state(json: &str) {
    call_window_hook_str("SOW_onLobbyState", json);
}
