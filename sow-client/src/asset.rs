                ...
                while let Ok(res) = self.tasks.map_rx.try_recv() {
                    match res {
                        MapDownloadEvent::Progress(downloaded_map_name, progress) => {
                            if Some(downloaded_map_name.clone()) == self.ui.app.main_menu_state.downloading_map_name {
                        ...
                        MapDownloadEvent::ThumbnailReady(map_name, bytes) => {
                            if let Ok(img) = image::load_from_memory(&bytes) {
                                ...
                                let texture = self.ui.egui_ctx.load_texture(&map_name, color_image, egui::TextureOptions::LINEAR);
                                self.ui.app.asset_loader.thumbnails.insert(map_name.clone(), texture);
                            } else {
                                log::warn!("Failed to decode thumbnail for {}", map_name);
                            }
                            self.ui.app.asset_loader.thumbnails_in_flight.remove(&map_name);
                        }
                        MapDownloadEvent::MapReady(map_name, bytes) => {
                            let mut valid = true;
                            if let Some(expected_md5) = self.ui.app.asset_loader.expected_md5s.get(&map_name) {
                            ...
                            if valid {
                                self.ui.app.asset_loader.maps.insert(map_name.clone(), bytes.clone());
                            } else {
