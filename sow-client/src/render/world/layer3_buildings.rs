    ...
    if let Some(snap) = &sim.current_snapshot {
        ...
        for b in rendered_buildings {
            ...
            if let Ok(egui::load::TexturePoll::Ready { texture }) = load_res {
                ...
                let r = ((player_color.r() as f32 * 0.40) + (255.0 * 0.60)) as u8;
                let g = ((player_color.g() as f32 * 0.40) + (255.0 * 0.60)) as u8;
                let b_val = ((player_color.b() as f32 * 0.40) + (255.0 * 0.60)) as u8;

                let alpha = if b.under_construction {
                    110
                } else {
                    170
                };
            ...
            if text_val != "1" {
                ...
                // Premium dark drop shadow
                painter.text(
                    bg_center + egui::vec2(1.0, 1.0),
                    egui::Align2::CENTER_CENTER,
                    &text_val,
                    egui::FontId::proportional(font_size),
                ...
                // Clean white level text matching building outlines
                painter.text(
                    bg_center,
                    egui::Align2::CENTER_CENTER,
                    &text_val,
