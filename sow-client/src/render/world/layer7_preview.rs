    ...
    if let Some(snap) = &sim.current_snapshot {
        ...
        if let Some(kind) = ui.app.hud_state.selected_building_kind {
            ...
            if col >= 0 && row >= 0 && col < sim.map_w as i32 && row < sim.map_h as i32 {
                ...
                let is_station_kind =
                    kind == sow_core::game::BuildingKind::City && upgrade_building.is_none();
                ...
                if let Some(start_idx) = snapped_idx {
                    ...
                    if is_station_kind && ui.last_preview_tile != Some(start_idx) {
                        ...
                        let eligible_stations: Vec<_> = buildings
                            .iter()
                            .filter(|b| {
                        ...
                        let temp_map = sow_core::map::GameMap {
                            ...
                            terrain: terrain
                                .iter()
                                .map(|&b| sow_core::map::MapTile::from_byte(b))
                                .collect(),
                        ...
                        if let Some(snapshot) = &sim.current_snapshot {
                            for rail in snapshot.railroads.iter() {
                                if true {
                                    if let (Some(&s_node), Some(&e_node)) =
                                        (rail.path.first(), rail.path.last())
                                    {
                        ...
                        let distance_from =
                            |start_tile: u32, dest_tile: u32, max_dist: usize| -> Option<usize> {
                        ...
                        for &(_, station) in &candidates {
                            ...
                            let already_reachable = connected_stations
                                .iter()
                                .any(|&s| distance_from(station.tile_idx, s, 3).is_some());
                            if already_reachable {
                                continue;
                            }

                            if let Some(path) = sow_core::building::railroad::find_rail_path(
                                &temp_map,
                                start_idx,
                                station.tile_idx,
                            ) {
                    ...
                    for path in &ui.cached_preview_paths {
                        ...
                        for rt in &rail_tiles {
                            ...
                            if is_water(tile_idx) {
                                ...
                                let bridge_color =
                                    egui::Color32::from_rgba_unmultiplied(197, 69, 72, 102);
                                for &[dx, dy, w, h] in bridge_rects {
                                    let world_x =
                                        c + 0.5 + (r as i32 % 2) as f32 * 0.5 + (dx as f32) / 2.0;
                                    let world_y = (r + 0.5) * 0.8660254_f32 + (dy as f32) / 2.0;
                                    let world_w = w as f32 / 2.0;
                                    let world_h = h as f32 / 2.0;

                                    let screen_x =
                                        (input.camera_x + world_x * input.camera_zoom) / sf;
                                    let screen_y =
                                        (input.camera_y + world_y * input.camera_zoom) / sf;
