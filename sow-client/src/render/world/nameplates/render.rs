    ...
    for vp in visible_players {
        ...
        if show_full {
            ...
            let (name_size, prepared_name, troops_galley) = match (cached_prepared, cached_troops) {
                ...
                _ => {
                    let prepared = sow_ui::widgets::prepare_name(painter, &display_name, &font_id);
                    let tg = crate::hud::nameplate::layout_nameplate_troops_galley(
                        painter,
                        troops_font_id.clone(),
                        &troops_str,
            ...
            if player.player_type == sow_core::player::PlayerType::Nation {
                ...
                let emoji_rect =
                    egui::Rect::from_center_size(avatar_center, egui::vec2(emoji_size, emoji_size));
            ...
            sow_ui::widgets::paint_prepared_name_with_glow(
                painter,
                egui::pos2(name_x, right_y),
                egui::Align2::LEFT_TOP,
                &prepared_name,
                vibrant_color,
                sow_ui::ui::theme::NAMEPLATE,
                Some(name_text_h),
            );
