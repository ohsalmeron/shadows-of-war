...
pub(crate) fn render(ui: &mut crate::app::UiState, sim: &crate::app::SimState, input: &crate::app::InputState, time: &crate::app::TimeState, gfx: &crate::app::GraphicsState, ctx: &RenderContext) {
    ...
    if let Some(snap) = &sim.current_snapshot {
            ...
            painter.ctx().data_mut(|d| d.insert_temp(time_id, current_time));

            let mut trains: Vec<VisualTrain> = painter.ctx().data(|d| d.get_temp(train_id)).unwrap_or_default();
            let mut anims: Vec<StationAnimation> = painter.ctx().data(|d| d.get_temp(anim_id)).unwrap_or_default();
                ...
                if let Some(rail) = snap.railroads.iter().find(|r| r.id == train.rail_id) {
                    if rail.path.len() > 1 {
                        ...
                        if arrived {
                            ...
                            let connected_rails: Vec<&sow_core::building::railroad::Railroad> = snap.railroads.iter()
                                .filter(|r| r.path.len() > 1 && (r.path[0] == arrival_tile || r.path[r.path.len() - 1] == arrival_tile))
                                .collect();
                ...
                if let Some(rail) = snap.railroads.iter().find(|r| r.id == train.rail_id) {
                    if rail.path.len() > 1 {
                        ...
                        for (i, &offset) in carriage_offsets.iter().enumerate() {
                            ...
                            painter.circle_stroke(screen_pos, radius, egui::Stroke::new(1.0_f32, stroke_col));
            ...
            for anim in &anims {
                ...
                if screen_x < -40.0 || screen_x > input.screen_w / sf + 40.0
