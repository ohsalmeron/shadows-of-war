...
* **CPU-Side Bit-Packing**: To permanently bypass Android GPU optimizer bugs, we stripped all neighbor-checking math out of the WGSL fragment shader. Instead, the CPU evaluates the 4 adjacent neighbors during the lockstep `dirty_tiles` loop in `O(1)` time, computes the border status, and packs a boolean flag into the 31st bit (`0x80000000`) of the `u32` texture payload. The GPU now does zero math and executes exactly 1 texture fetch per pixel, ensuring 100% compatibility across all devices.
